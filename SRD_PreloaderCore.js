 ...
 * @param Custom Background
 * @text カスタム背景
 * @desc プリロードシーン用のカスタム背景画像
 * /img/SumRndmDde/preload/ に配置してください。デフォルトの場合、無入力
 * @default
 *
 * @param Loading Text
 * @text 読み込み中テキスト
 * @desc どのファイルがロードされるかを説明するために使用されるテキストのフォーマット。ファイルパスを表すには%1を使用
 * @default 読み込み中 %1
 *
 * @param Complete Text
 * @text 完了テキスト
 * @desc 全てのロードが完了した時の表示テキスト
 * @default ロード完了
 *
 * @param Use Fade Transitions
 * @text フェードトランジションを使用
 * @type boolean
 * @desc 'true'の場合、タイトルシーンとプリロードシーンの間にフェードが発生
 * @default true
 *
 * @param Load Font Size
 * @text フォントサイズを読み込む
 ...
 * @min 0
 * @decimals 0
 * @desc ロードURLを表示するときのテキストのフォント・サイズ
 * @default 28
 *
 * @param Gauge Back Color
 * @text ゲージ背景色
 ...
 * @param Gauge Main Color
 * @text ゲージメイン色
 * @desc ロードゲージのメイン部分に使用される色
 * @default rgba(255, 0, 0, 0.4)
 *
 * @param Custom Preloads
 * @text プリロードカスタム
 * @type Struct<CustomPreloads>[]
 * @desc ゲームの全てのカスタムプリロードのリスト
 * @default []
 *
 * @param Audio Preloads
 * @text オプリロードオーディ
 ...
 * @param Cache Audio
 * @text キャッシュオーディオ
 * @desc プリロードオーディオオブジェクトを保存
 * キャッシングを使用する全てのオーディオフォルダをリストアップします。
 * @default bgm, bgs, me
 * @parent Audio Preloads
 *
 * @param Preload BGM
 ...
 * @option custom:
 * @option none
 * @desc プリロードされているBGMを決定
 * カスタムファイルを使用する場合'custom: f1, f2, ...'と入力
 * @default important
 * @parent Audio Preloads
 *
 * @param Preload BGS
 ...
 * @option custom:
 * @option none
 * @desc プリロードされているシステムイメージを決定。選択肢は以下
 * カスタムファイルを使用する場合'custom: f1, f2, ...'と入力
 * @default none
 * @parent Image Preloads
 *
 ...
 * @option custom:
 * @option none
 * @desc プリロードされるアニメーションを決定。選択肢は以下
 * カスタムファイルを使用する場合'custom: f1, f2, ...'と入力
 * @default none
 * @parent Image Preloads
 *
 ...
 * @option custom:
 * @option none
 * @desc プリロードされるバトルバック1を決定。選択肢は以下
 * カスタムファイルを使用する場合'custom: f1, f2, ...'と入力
 * @default none
 * @parent Image Preloads
 *
 ...
 * @option custom:
 * @option none
 * @desc プリロードされているバトルバック2を決定。選択肢は以下
 * カスタムファイルを使用する場合'custom: f1, f2, ...'と入力
 * @default none
 * @parent Image Preloads
 *
 ...
 * @option custom:
 * @option none
 * @desc プリロードされているキャラクターを決定。選択肢は以下
 * カスタムファイルを使用する場合'custom: f1, f2, ...'と入力
 * @default none
 * @parent Image Preloads
 *
 ...
 *
 * 元プラグイン: http://sumrndm.site/preloader-core/
 *
 *
 * Preloader Core
 * Version 1.10
 ...
 * http://sumrndm.site/game-upgrade/
 *
 * プラグイン管理でこのプラグインをSRD_GameUpgradeの下側に配置してください。
 *
 ...
 *
 * 全てのaudioとimgフォルダには、
 * ファイルをプリロードするための4つの選択肢があります。
 * none, custom, important, all
 *
 *
 *   none
 *
 * プリロードが "none "に設定されている場合、
 * そのフォルダのファイルはプリロードされません。
 *
 *
 *   custom: f1, f2, ...
 *
 * 'custom'に設定すると、特定のファイルを選択してプリロードすることができます。
 * ファイル名を入力し、拡張子は付けず、カンマで区切ってください。
 * 例 - custom: Battle1, Battle2, Theme6
 *
 *
 *   important
 *
 * 'important'に設定すると、
 * 'important'と判断されたファイルがプリロードされます。
 * 何が'important'とみなされるかはフォルダ毎に異なります。
 * 例えば、重要なBGMにはデータベースで指定されたものが含まれ、
 * 重要なアニメーション画像にはアニメーションで使用される全ての画像が含まれます。
 *
 *
 *   all
 *
 * フォルダ内の全てのファイルがプリロードされます。
 * この機能はNode.jsがサポートされているプラットフォームでのみ使用できます。
 *
 *
 * ==========================================================================
 *  プラグインコマンド
 * ==========================================================================
 *
 * ゲーム内のオーディオや画像を手動でプリロードしたい場合、
 * 以下のプラグインコマンドを使用することができます。
 *
 *
 *   PreloadAudio [folder] [name]
 *
 * 'folder'をaudioフォルダの一つに置き換え、
 * 'name'をそのフォルダ内のファイル名の1つに置き換えます。
 * 例 - PreloadAudio bgm Theme6
 *
 *
 *   PreloadImage [folder] [name] [hue]
 *
 * 'folder'をimgフォルダの1つに置き換え、
 * 'name'をそのフォルダ内のファイル名の1つに置き換えます。
 * また、オプションの'hue'引数を使用して、
 * プリロードされた画像に特定色相をプリロードさせることもできます。
 * 例 - PreloadImage titles1 Book 0
 *
 *
 * ==========================================================================
 *  ヘルプファイルの終わり
 ...
 * @param Preload Path
 * @text プリロードパス
 * @desc /img/フォルダにプリロードする画像を参照するためのカスタムフォルダパス
 * @default
 *
 * @param Preload Images
 * @text プリロード画像
