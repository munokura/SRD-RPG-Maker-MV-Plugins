 ...
 */
/*:ja
 * @plugindesc ランダムエンカウントの戦闘イントロにコモンイベントを実行できます。
 * @author SumRndmDde
 *
 * @param Common Event ID
 * @text コモンイベントID
 * @type common_event
 * @desc 戦闘イントロで実行されるコモンイベントID
 * @default 1
 *
 * @param Auto Transition
 * @text 戦闘フェード移行
 ...
 *
 * 元プラグイン: http://sumrndm.site/common-event-battle-intro/
 *
 *
 * Common Event Battle Intro
 * Version 1.00
 ...
 *
 *
 * ランダムエンカウントの戦闘イントロにコモンイベントを実行できます。
 *
 * 'Common Event ID'パラメータで、
 * マップでランダムエンカウント前に実行したいコモンイベントに設定するだけです。
 *
 *
 * ==========================================================================
 *  コモンイベントの変更
 ...
 *
 * 以下のプラグインコマンドで
 * 戦闘イントロで実行するコモンイベントを変更します。
 *
 *   SetCommonEventBattleIntro [id]
 *
 * [id]にコモンイベントのIDを設定します。
 * 例:
 *
 *   SetCommonEventBattleIntro 3
 *
 *
 * デフォルトに戻したい場合、下記プラグインコマンドを使用してください。
 *
 *   ResetCommonEventBattleIntro
 *
 *
 * ==========================================================================
 *  コモンイベント戦闘イントロをマップに割り当てる
 * ==========================================================================
 *
 * マップのメモタグ内で、以下を使用すると、
 * マップ毎にコモンイベントを設定できます。
 ...
 *   <CE Battle Intro: [id]>
 *
 * [id]を使用するコモンイベントのIDに置き換えてください。
 *
 * マップにカスタム戦闘イントロがある場合、
 * パラメータ/プラグインコマンドで定義された戦闘イントロよりも優先されます。
 * マップのコモンイベント戦闘イントロを無効にする場合、
 * 下記のプラグインコマンドを使用します。
 *
 *   DisableMapCommonEventBattleIntro
 *
 * 再度有効にしたい場合、下記プラグインコマンドを使用してください。
 *
 *   EnableMapCommonEventBattleIntro
 *
 *
 ...
 * ==========================================================================
 *
 * デフォルトでは、"Auto Transition"パラメータがtrueに設定されていて、
 * 戦闘イントロの最後には、画面が黒くフェードアウトして戦闘BGMが流れます。
 *
 * falseに設定されている場合でも、
 * コモンイベント内からプラグインコマンドでこの動作をさせることができます。
 *   BattleIntroTransition
 *
 *
 * オリジナルの戦闘イントロや
 * 他のプラグインで作成した戦闘イントロを使いたい場合、
 * 下記のプラグインコマンドを使って呼び出すことができます。
 *   StartNormalBattleIntro
 *
 * ※他のエンカウント演出プラグインを併用する場合、
 * プラグイン管理でこのプラグインを他のプラグインの下側に配置してください。
 * また、この場合にコモンイベントの内容によって
 * 正常に処理されない事がある問題を確認しています。
 *   問題例:「文章の表示」でウィンドウが表示されず、
 *          ボタンを押すと戦闘に移行します。
 *
 * 解決例:コモンイベント内容
 * ◆プラグインコマンド：StartNormalBattleIntro
 * ◆文章：なし, ウィンドウ, 下
 * ：　　：敵だ！
 *
 * 上記のように、プラグインコマンドを最初に実行することで、
 * 戦闘ウィンドウに文章が表示されます。
 *
 * ==========================================================================
 *  ヘルプファイルの終わり
