 ...
 */
/*:ja
 * @plugindesc プレイヤーがリトライするための必要なコストを設定できます。
 * @author SumRndmDde
 *
 * @param Default Gold Cost
 * @text デフォルトの所持金コスト
 * @desc リトライにかかる所持金のデフォルト量
 * これは数字かJavaScriptのコードが使えます
 * @default 10
 *
 * @param Default Item Cost
 * @text デフォルト項目コスト
 * @desc リトライにかかるアイテムのデフォルトのセット
 * 例: [item1, # of item1], [item2, # of item2], ...
 * @default
 *
 * @param Default Retry Cond.
 * @text デフォルトの再試行条件
 * @desc リトライするためのJavaScriptの条件を指定します
 * 'true'にすると常に有効になります
 * @default true
 *
 * @param Default Retry Eval
 * @text デフォルトの再試行評価
 * @desc プレイヤーがリトライしたときに実行される JavaScript の評価
 * 使用しない場合、無入力
 * @default
 *
 * @param Default Retry Text
 * @text デフォルトの再試行テキスト
 * @desc JavaScriptのリトライ条件に表示されるテキスト
 * 使用しない場合、無入力
 * @default
 *
 * @param == Texts ==
 ...
 * @param Gold Icon
 * @text 所持金アイコン
 * @desc 所持金コストを表すために使用されるアイコン
 * @default 313
 *
 * @param Retry Cost Text
 * @text 再試行コスト テキスト
 * @desc 「リトライコスト」の表示テキスト
 * @default リトライコスト
 *
 * @param Gold Text
 * @text 所持金のテキスト
 * @desc リトライコストの「所持金」表示テキスト
 * @default 所持金
 *
 * @help
 * 翻訳:ムノクラ
 ...
 *
 * 元プラグイン: http://sumrndm.site/retry-costs/
 *
 *
 * Retry Costs
 * Version 1.01
 ...
 *   SetRetryItemCost [items]
 *
 * リトライに必要なアイテムコストを設定します。
 *
 *
 *   SetRetryCondition [javascript code]
 *
 * JavaScriptのリトライ条件を設定します。
 *
 *
 *   SetRetryEval [javascript code]
 *
 * リトライに必要な JavaScript eval を設定します。
 *
 *
 *   SetRetryText [text]
 *
