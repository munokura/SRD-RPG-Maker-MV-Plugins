 ...
 */
/*:ja
 * @plugindesc ゲームオーバーにオプションを追加します。コモンイベントを実行でき、リトライ機能が追加されます。
 * @author SumRndmDde
 *
 * @param Default Image
 * @text デフォルト画像
 * @desc ゲームオーバーシーンで表示するデフォルト画像
 * (/img/SumRndmDde/gameover/内) デフォルトを使う場合は無入力
 * @default
 *
 * @param == Command Window ==
 * @text -- コマンドウィンドウ --
 ...
 * @option 終了
 * @value end
 * @desc ゲームオーバーコマンドウィンドウを開く段階
 * 開始:start / 中間:middle / 終了:end
 * @default end
 *
 * @param Allow Retry Command
 * @text リトライを許可するコマンド
 ...
 * @on 許可
 * @off 禁止
 * @desc プレイヤーはリトライコマンドを選択し、最初から戦闘を再開可能
 * 許可:true / 禁止:false
 * @default true
 *
 * @param Command Window X
 * @text コマンドウィンドウX位置
 * @desc ゲームオーバーコマンドウィンドウのX位置
 * 数字かJavaScriptコードが使えます。
 * @default (Graphics.boxWidth - this._commandWindow.width) / 2
 *
 * @param Command Window Y
 * @text コマンドウィンドウY位置
 * @desc ゲームオーバーコマンドウィンドウのY位置
 * 数字かJavaScriptコードが使えます。
 * @default (Graphics.boxHeight - this._commandWindow.height) / 2
 *
 * @param == Command Texts ==
 ...
 * @param Retry Command Text
 * @text リトライコマンドテキスト
 * @desc ゲームオーバーコマンドウィンドウのリトライコマンドの表示テキスト
 * @default リトライ
 *
 * @param Load Command Text
 * @text 読み込みコマンドテキスト
 * @desc ゲームオーバーコマンドウィンドウのコンティニューの表示テキスト
 * @default コンティニュー
 *
 * @param Title Command Text
 * @text タイトルコマンドテキスト
 * @desc ゲームオーバーコマンドウィンドウのタイトルコマンドの表示テキスト
 * @default ゲーム終了
 *
 * @param == Command Events ==
 * @text -- コマンドイベント
 ...
 * @text 開始コマンドイベント
 * @type common_event
 * @desc ゲームオーバーシーンの開始時に実行するコモンイベントID
 * 無効の場合は0
 * @default 0
 *
 * @param Middle Common Event
 * @type common_event
 * @text 中間コモンイベント
 * @desc ゲームオーバーシーンの途中で実行するコモンイベントID
 * 無効の場合は0
 * @default 0
 *
 * @param End Common Event
 * @type common_event
 * @text 終了コマンドイベント
 * @desc ゲームオーバーシーンの最後に実行するコモンイベントID
 * 無効の場合は0
 * @default 0
 *
 * @help
 ...
 *
 * 元プラグイン: http://sumrndm.site/game-over-core/
 *
 * Game Over Core
 * Version 1.07
 * SumRndmDde
 *
 *
 * このプラグインは、ゲームオーバーシーンのオプションを増やします。
 * コモンイベントを呼び出すことができ、コマンドウィンドウが追加され、
 * ゲームのロード、リトライ、タイトル画面の復帰が可能です。
 *
 *
 * ==========================================================================
 *  プラグインコマンド
 ...
 *
 * ゲームオーバーシーンの様々な要素は、
 * 特定のプラグインコマンドを使って操作することができます。
 * 以下にその一覧を示します。
 *
 *
 *   SetGameOverImage [filename]
 *
 * 下記に保存されている指定ファイルにゲームオーバー画像を設定します。
 * /img/SumRndmDde/gameover/
 *
 *
 *   SetGameOverstartCommonEvent [id]
 *
 * ゲームオーバー開始(画像表示前)コモンイベントIDを設定します。
 * 無効にするには0を設定します。
 *
 *
 *   SetGameOverMiddleCommonEvent [id]
 *
 * ゲームオーバー中間(画像表示完了)コモンイベントIDを設定します。
 * 無効にするには0を設定します。
 *
 *
 *   SetGameOverEndCommonEvent [id]
 *
 * ゲームオーバー終了(画像消去完了)コモンイベントIDを設定します。
 * 無効にするには0を設定します。
 *
 *
 *   ResetGameOverImage
 *
 * ゲームオーバー画像をパラメータで指定したデフォルト値にリセットします。
 *
 *
 *   ResetGameOverCommonEvents
 *
 * ゲームオーバーのコモンイベントを全てリセットして、
 * パラメータで指定したデフォルトに戻します。
 *
 *
 * ==========================================================================
 *  戦闘リトライの強制
 ...
 *
 * 戦闘リトライをイベント化したい場合、
 * 以下のプラグインコマンドを使用して戦闘リトライを強制的に行います。
 *
 *   RetryBattle
 *
 *
