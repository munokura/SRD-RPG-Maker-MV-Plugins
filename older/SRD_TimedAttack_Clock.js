 ...
 */
/*:ja
 * @plugindesc 時計の針が文字盤の周りを回転するのと似たタイミング攻撃を追加します。
 * @author SumRndmDde
 * 
 * @param Sound Effect
 * @text サウンドエフェクト
 ...
 * @require 1
 * @dir audio/se
 * @desc デフォルトのSE。/audio/se/ 内のSEのファイル名
 *
 * @param Background Image
 * @text 背景画像
 * @desc デフォルトの背景画像。/img/SumRndmDde/tas/ 内の背景画像のファイル名
 * @require 1
 * @default
 * 
 * @param Repeat Type
 ...
 * @param Speed
 * @text 速度
 * @desc 速度がデフォルトの繰り返しタイプ。正の値または JavaScript 式を入力
 * @default 5
 * 
 * @param Length
 * @text 長さ
 ...
 * @param Height
 * @text 高さ
 * @desc 高さがデフォルトの繰り返しタイプ。正の値を入力
 * @type number
 * @default 150
 * 
 * @param Thickness
 * @text 厚さ
 * @desc リングの厚さがデフォルトの繰り返しタイプ。正の値を入力
 * @type number
 * @default 4
 * 
 * @param Start Position
 * @text 開始位置
 * @desc リングの厚さがデフォルトの繰り返しタイプ。正の角度を入力
 * @type number
 * @default 0
 * 
 * @param End Position
 * @text 終了位置
 * @desc デフォルトの開始位置。正の角度を入力
 * @type number
 * @default 360
 * 
 * @param Hand Color
 * @text 手の色
 * @desc デフォルトの手の色。JavaScript または HTMLの色コード
 * @default #ffffff
 * 
 * @param Base Color
 * @text ベース色
 * @desc デフォルトの基本色。JavaScript または HTMLの色コード
 * @default #000000
 * 
 * @param Fade Color
 * @text フェード色
 * @desc デフォルトのフェード色。JavaScript または HTMLの色コード
 * @default #cccccc
 * 
 * @param Background Color
 * @text 背景色
 * @desc デフォルトの背景色。JavaScript または HTMLの色コード
 * @default #5e6b6e
 * 
 * @param Outline Color
 * @text アウトライン色
 ...
 * @param Target 1
 * @text 対象1
 * @desc デフォルトの対象1。次の式を使用します: 開始,終了,色,パワー
 * @default 165, 195, #F25B60, 1
 * 
 * @param Target 2
 * @text 対象2
 * @desc デフォルトの対象2。次の式を使用します: start、終了、色、パワー
 * @default
 * 
 * @param Target 3
 * @text 対象3
 * @desc デフォルトの対象3。次の式を使用します: 開始,終了,色,パワー
 * @default
 * 
 * @param Target 4
 * @text 対象4
 * @desc デフォルトの対象4。次の式を使用します: 開始,終了,色,パワー
 * @default
 * 
 * @param Target 5
 * @text 対象5
 * @desc デフォルトの対象5。次の式を使用します: 開始,終了,色,パワー
 * @default
 * 
 * @param Cooldown
 * @text クールダウン
 * @desc デフォルトのクールダウン。正の値を入力
 * @type number
 * @default 16
 * 
 * @param Flash Rate
 * @text フラッシュレート
 * @desc デフォルトのフラッシュレート。正の値を入力
 * @type number
 * @default 16
 * 
 * @param Flash Time
 ...
 * 
 * 元プラグイン: http://sumrndm.site/timed-attack-clock/
 * 
 * Timed Attack: Clock
 * Version 1.10
 * SumRndmDde
 ...
 * 
 * 時計の針が時計の文字盤の周りを回転する動きに基づいたタイミング攻撃です。
 * 
 * プレイヤーは「時計の針」が対象領域の上に来たときに「OK」ボタンを押すか、
 * 画面をタップすることでパワーを得ます。
 * 
 * ==========================================================================
 ...
 * (a.atk * 4 - b.def * 2) * ($gameTemp.tas_power)
 * 
 * この例では「タイミング攻撃」でパーフェクトヒットを獲得すると
 * 最大ダメージが得られます。
 * パーフェクト以外のヒットを獲得すると、
 ...
 * <End Timed Attack>
 * 
 * 上記のように、各タイミング攻撃別に複数の対象を設定できます。
 * 
 * 各タイミング攻撃別に最大 5 つの対象を設定できます。
 * 
 * 各対象には 4 つの要素があります。
 * 
 * - 開始位置
 ...
 * 順番にカンマで区切る必要があります。
 * 
 * 開始/終了位置は角度に基づきます (例: 0 - 360)
 * 
 * 色は JavaScript または HTML の色コードが使えます。
 * 
 * パワーは、ヒットする効果を決定します。
 * 1 がデフォルトの値ですが、パワーを増減できます。
 * 
 *
 * ==========================================================================
 *  時計型のタイミング攻撃要素
