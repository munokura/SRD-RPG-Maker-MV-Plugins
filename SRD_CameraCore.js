 ...
 */
/*:ja
 * @plugindesc フォーカスやズーム等の様々なカメラの動きを事前設定できます。
 * @author SumRndmDde
 *
 * @param Default Zoom
 * @text デフォルトのズーム
 ...
 * @on 有効
 * @off 無効
 * @desc "ピクチャの表示"で作成されたピクチャをカメラでズーム
 * 有効:true / 無効:false
 * @default true
 *
 * @param Margin Formula
 * @text マージン式
 * @desc ズームアウト時のレンダリングマージンを決定する式
 * @default (Graphics.width / scale) - Graphics.width
 *
 * @param Fix Black Lines
 * @text 黒線の修正
 ...
 * @on 有効
 * @off 無効
 * @desc YEP_CoreEngine使用時、黒線が表示される不具合を修正
 * 有効:true / 無効:false
 * @default false
 *
 * @help
 ...
 *
 * 元プラグイン: http://sumrndm.site/camera-core/
 *
 *
 * Camera Core
 * Version 1.05
 ...
 * マップ上にXとYの座標を挿入して、
 * カメラがその座標にフォーカスします。
 * durationは、カメラが遷移するのにかかるフレーム数を決定します。
 *
 *
 *   FocusCamera event [id] [duration]
 *
 * idを現在のマップ上のイベントのIDに置き換えて、
 * カメラを特定のイベントにフォーカスします。
 * durationは、カメラが遷移するのにかかるフレーム数を決定します。
 *
 *
 *   FocusCamera follower [id] [duration]
 *
 * idを現在のパーティ内のフォロワーのインデックスに置き換えて、
 * カメラが特定のフォロワーにフォーカスします。
 * durationは、カメラが遷移するのにかかるフレーム数を決定します。
 *
 *
 *   FocusCamera player [duration]
 ...
 * ==========================================================================
 *
 * カメラを拡大・縮小するには、以下のコマンドを使用します。
 *
 *
 *   ZoomIn [scale] [duration]
 *
 * scaleは、カメラをズームさせたい拡縮率を設定します。
 * scaleを1に設定すると通常の拡縮率になり、
 * 1より大きくすると拡大します。
 * durationは、カメラが遷移するのにかかるフレーム数を決定します。
 *
 *
 ...
 * ==========================================================================
 *
 * カメラをずらしたい場合、以下のプラグインコマンドを使用します。
 *
 *   ShiftCamera [x-shift] [y-shift] [duration]
 *
 * カメラを指定のx/yブロック移動します。カメラのフォーカスは変わりません。
 * 例えば、カメラがプレイヤーにフォーカスを合わせていて、
 * カメラを2マス上に移動させた場合、カメラはプレイヤーを追い続けますが、
 * 2マス上に移動します。
 *
 *
 * ==========================================================================
 ...
 * ==========================================================================
 *
 * カメラの移動が終わる前にイベントを'待機'させたい場合、
 * 以下のプラグインコマンドを使用します。
 *
 *   WaitForCamera
 *
 * カメラのフォーカスとズームが完了するまでイベント処理を停止します。
 *
 *
 *   WaitForCameraFocus
 *
 * カメラが新しいフォーカスに移動を完了するのを待ちます。
 *
 *
 *   WaitForCameraZoom
 *
 * カメラのズーム動作が終了するのを待ちます。
 *
 *
 * ==========================================================================
 *  例
